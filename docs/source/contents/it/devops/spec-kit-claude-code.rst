...
This guide shows how to use `Anthropic Claude Code <https://www.anthropic.com/claude-code>`_
...
- Claude Code provides an agentic coding assistant that can plan, edit files,
...
Anthropic also provides native binaries. See:
...
Once the spec is in place, let Claude Code plan the implementation and create
the first changes.
...
- Ensure the built-in commands like ``/specify``, ``/plan``, and ``/tasks`` are
  available. Then invoke ``/specify`` and provide the concrete requirements for
  what you want to build. Focus on what and why, not the tech stack yet.
